...
static int tp_test_wait_for_publication(tp_client_t *client, tp_publication_t *pub)
{
    int64_t deadline = tp_clock_now_ns() + 2 * 1000 * 1000 * 1000LL;
    ...
    while (tp_clock_now_ns() < deadline)
    {
        if (aeron_publication_is_connected(tp_publication_handle(pub)))
        {
            return 0;
        }
        tp_client_do_work(client);
...
static int tp_test_wait_for_subscription(tp_client_t *client, tp_subscription_t *sub)
{
    int64_t deadline = tp_clock_now_ns() + 2 * 1000 * 1000 * 1000LL;
    ...
    while (tp_clock_now_ns() < deadline)
    {
        if (aeron_subscription_is_connected(tp_subscription_handle(sub)))
        {
            return 0;
        }
        tp_client_do_work(client);
...
static void tp_test_update_activity_timestamp(int fd, uint64_t activity_ns)
{
    uint8_t buffer[TP_SUPERBLOCK_SIZE_BYTES];
    struct tensor_pool_shmRegionSuperblock block;
    ...
    tensor_pool_shmRegionSuperblock_wrap_for_encode(&block, (char *)buffer, 0, sizeof(buffer));
    tensor_pool_shmRegionSuperblock_set_activityTimestampNs(&block, activity_ns);
...
void tp_test_rate_limit(void)
{
    ...
    while (tp_clock_now_ns() < deadline)
    {
        if (aeron_subscription_is_connected(tp_subscription_handle(descriptor_sub)) &&
            aeron_publication_is_connected(tp_publication_handle(entry->descriptor_publication)))
        {
            break;
        }
        tp_client_do_work(&client);
...
void tp_test_qos_liveness(void)
{
    ...
    old_ns = (uint64_t)tp_clock_now_ns() - (stale_ns / 2);
...
void tp_test_qos_drop_counts(void)
{
    ...
    int header_fd = -1;
    int pool_fd = -1;
    char header_path[] = "/tmp/tp_qos_drop_headerXXXXXX";
    char pool_path[] = "/tmp/tp_qos_drop_poolXXXXXX";
    char header_uri[256];
    char pool_uri[256];
    ...
    uint64_t drops_gap = 0;
    uint64_t drops_late = 0;
    int result = -1;
    int64_t deadline;
    ...
    tp_test_write_superblock(header_fd, 82001, 1, tensor_pool_regionType_HEADER_RING, 0, 4, TP_HEADER_SLOT_BYTES, 0);
    tp_test_write_superblock(pool_fd, 82001, 1, tensor_pool_regionType_PAYLOAD_POOL, 1, 4, TP_NULL_U32, 64);
    ...
    tensor_pool_frameDescriptor_set_streamId(&descriptor, 82001);
    tensor_pool_frameDescriptor_set_epoch(&descriptor, 1);
    tensor_pool_frameDescriptor_set_timestampNs(&descriptor, 0);
    tensor_pool_frameDescriptor_set_metaVersion(&descriptor, 0);
...
void tp_test_epoch_remap(void)
{
    ...
    int header_fd = -1;
    int pool_fd = -1;
    char header_path[] = "/tmp/tp_epoch_remap_headerXXXXXX";
    char pool_path[] = "/tmp/tp_epoch_remap_poolXXXXXX";
    char header_uri[256];
    char pool_uri[256];
    uint8_t buffer[512];
    struct tensor_pool_messageHeader header;
    ...
    tp_test_write_superblock(header_fd, 83001, 1, tensor_pool_regionType_HEADER_RING, 0, 4, TP_HEADER_SLOT_BYTES, 0);
    tp_test_write_superblock(pool_fd, 83001, 1, tensor_pool_regionType_PAYLOAD_POOL, 1, 4, TP_NULL_U32, 64);
    ...
    tensor_pool_shmPoolAnnounce_wrap_for_encode(&announce, (char *)buffer, tensor_pool_messageHeader_encoded_length(), sizeof(buffer));
    tensor_pool_shmPoolAnnounce_set_streamId(&announce, 83001);
    tensor_pool_shmPoolAnnounce_set_producerId(&announce, 2);
    tensor_pool_shmPoolAnnounce_set_epoch(&announce, 1);
    tensor_pool_shmPoolAnnounce_set_layoutVersion(&announce, 1);
    tensor_pool_shmPoolAnnounce_set_headerNslots(&announce, 4);
    ...
    tp_test_write_superblock(header_fd, 83001, 2, tensor_pool_regionType_HEADER_RING, 0, 4, TP_HEADER_SLOT_BYTES, 0);
    tp_test_write_superblock(pool_fd, 83001, 2, tensor_pool_regionType_PAYLOAD_POOL, 1, 4, TP_NULL_U32, 64);
...
void tp_test_activity_liveness(void)
{
    ...
    char pool_path[] = "/tmp/tp_activity_poolXXXXXX";
    char header_uri[256];
    char pool_uri[256];
    tp_consumer_config_t config;
    tp_consumer_pool_config_t pool_cfg;
    ...
    tp_test_write_superblock(header_fd, 74001, 1, tensor_pool_regionType_HEADER_RING, 0, 4, TP_HEADER_SLOT_BYTES, 0);
    tp_test_write_superblock(pool_fd, 74001, 1, tensor_pool_regionType_PAYLOAD_POOL, 1, 4, TP_NULL_U32, 64);
...
void tp_test_consumer_fallback_state(void)
{
    ...
    const char *fallback_uri = "bridge://pool";
    int result = -1;
    int64_t deadline;
    ...
    tensor_pool_consumerConfig_set_streamId(&cfg, 75001);
    tensor_pool_consumerConfig_set_consumerId(&cfg, 6);
    ...
    tensor_pool_consumerConfig_put_payloadFallbackUri(&cfg, fallback_uri, strlen(fallback_uri));
...
void tp_test_consumer_fallback_recover(void)
{
    ...
    int header_fd = -1;
    int pool_fd = -1;
    char header_path[] = "/tmp/tp_fallback_headerXXXXXX";
    char pool_path[] = "/tmp/tp_fallback_poolXXXXXX";
    char header_uri[256];
    char pool_uri[256];
    uint64_t now_ns;
    int result = -1;
    ...
    tp_test_write_superblock(header_fd, 75002, 1, tensor_pool_regionType_HEADER_RING, 0, 4, TP_HEADER_SLOT_BYTES, 0);
    tp_test_write_superblock(pool_fd, 75002, 1, tensor_pool_regionType_PAYLOAD_POOL, 1, 4, TP_NULL_U32, 64);
    ...
    tensor_pool_consumerConfig_wrap_for_encode(&cfg, (char *)buffer, tensor_pool_messageHeader_encoded_length(), sizeof(buffer));
    tensor_pool_consumerConfig_set_streamId(&cfg, 75002);
    ...
    tensor_pool_shmPoolAnnounce_wrap_for_encode(&announce, (char *)buffer, tensor_pool_messageHeader_encoded_length(), sizeof(buffer));
    tensor_pool_shmPoolAnnounce_set_streamId(&announce, 75002);
    tensor_pool_shmPoolAnnounce_set_producerId(&announce, 2);
    tensor_pool_shmPoolAnnounce_set_epoch(&announce, 1);
    tensor_pool_shmPoolAnnounce_set_layoutVersion(&announce, 1);
    tensor_pool_shmPoolAnnounce_set_headerNslots(&announce, 4);
...
cleanup:
    tp_fragment_assembler_close(&progress_poller.assembler);
    if (progress_pub)
    {
        tp_publication_close(&progress_pub);
    }
    if (progress_sub)
    {
