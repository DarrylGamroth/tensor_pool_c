...
ready=false
ready_waited=false
READY_ATTACH_TIMEOUT_MS="${READY_ATTACH_TIMEOUT_MS:-5000}"
READY_INITIAL_DELAY_S="${READY_INITIAL_DELAY_S:-5}"
if (( READY_INITIAL_DELAY_S > 0 )); then
  sleep "$READY_INITIAL_DELAY_S"
fi
SECONDS=0
while (( SECONDS < READY_TIMEOUT_S )); do
  ...
  TP_EXAMPLE_REQUIRE_PROGRESS= \
  TP_EXAMPLE_POLL_PROGRESS= \
  TP_EXAMPLE_PROGRESS_MIN= \
