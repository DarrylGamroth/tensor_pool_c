...
int main(int argc, char **argv)
{
    ...
    fprintf(stderr,
        ...
        tp_context_get_descriptor_channel(client_context),
        tp_context_get_descriptor_stream_id(client_context));
    ...
    tp_context_set_error_handler(client_context, on_error, &error_state);
    ...
    if (tp_driver_attach_request_init(&request, stream_id, TP_ROLE_CONSUMER) < 0)
    {
        if (!silent_attach)
        {
    ...
    if (tp_consumer_context_init_default(&consumer_context, info.stream_id, resolved_client_id, false) < 0)
    {
