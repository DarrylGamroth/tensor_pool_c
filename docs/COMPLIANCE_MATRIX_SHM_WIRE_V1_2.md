...
| Section | Status | Evidence / Notes |
| --- | --- | --- |
| 6. SHM Region Structure | Compliant | Superblock validation and ShmPoolAnnounce cross-checks in `src/common/tp_shm.c` and `src/client/tp_consumer.c`. |
...
| 7.1 ShmRegionSuperblock | Compliant | Magic/layout/epoch/stream/pool/type checks in `src/common/tp_shm.c`, linked to ShmPoolAnnounce in `src/client/tp_consumer.c`. |
...
| 10.3.1 DataSourceAnnounce | Compliant | Encode/decode in `src/client/tp_control.c` and `src/client/tp_control_adapter.c`. |
| 10.3.2 DataSourceMeta | Compliant | Encode/decode in `src/client/tp_control.c` and `src/client/tp_control_adapter.c`. |
| 10.3.3 Meta blobs | Compliant | MetaBlob announce/chunk/complete encode/decode in `src/client/tp_control.c` and `src/client/tp_control_adapter.c`. |
...
| 15.2 Epoch Lifecycle | Compliant | ShmPoolAnnounce epoch tracking, remap, and drop behavior in `src/client/tp_consumer.c`. |
| 15.3 Commit Protocol Edge Cases | Compliant | Seqlock stability and seq mismatch handled as drops. |
| 15.4 Overwrite and Drop Accounting | Compliant | Drops tracked in `src/client/tp_consumer.c` with accessor in `include/tensor_pool/client/tp_consumer.h`. |
...
| 15.7 Timebase | Compliant | ShmPoolAnnounce clock-domain and join-time rules enforced in `src/client/tp_consumer.c`. |
...
| Section | Status | Evidence / Notes |
| --- | --- | --- |
| 5. Requirements and Constraints | Compliant | JoinBarrier hot path is allocation-free after init in `src/common/tp_join_barrier.c`. |
| 6. MergeMap | Compliant | Encode/decode helpers and rule validation in `src/common/tp_merge_map.c`. |
| 6.4 Stability and Epoch Handling | Compliant | MergeMap registry invalidates prior epochs in `src/common/tp_merge_map.c`. |
| 7. SequenceJoinBarrier | Compliant | Readiness, staleness handling, and observed/processed cursor checks in `src/common/tp_join_barrier.c`. |
| 8. TimestampJoinBarrier | Compliant | Clock domain and monotonic out_time enforcement in `src/common/tp_join_barrier.c`. |
| 9. LatestValueJoinBarrier | Compliant | Selection collection + invalidation and epoch gating in `src/common/tp_join_barrier.c`. |
