...
| W-7.1-3 | 7.1 | Liveness via `activity_timestamp_ns` updates + stale remap | `src/tp_producer.c`, `src/tp_consumer.c`, `src/tp_shm.c` | `tests/test_tp_pollers.c` | Partial | Cadence policy not enforced |
...
| W-8.3-1 | 8.3 | Seqlock commit bit semantics + stable read | `src/tp_consumer.c`, `src/tp_producer.c`, `src/tp_slot.c` | `tests/test_tp_rollover.c` | Partial | No explicit DMA flush hooks |
...
| W-10.4-1 | 10.4 | QoS message encode/decode | `src/tp_qos.c`, `src/tp_control_adapter.c` | `tests/test_tp_pollers.c` | Partial | Publish cadence policy not enforced |
...
| W-15.3-1 | 15.3 | Commit protocol edge cases | `src/tp_consumer.c` | `tests/test_tp_rollover.c` | Partial | No explicit seq_commit regression handling |
...
| W-15.8-1 | 15.8 | Enum/type registry versioning | n/a | n/a | Partial | No registry/version checks |
...
| W-15.14-1 | 15.14 | Liveness: ShmPoolAnnounce freshness, pid/activity checks | `src/tp_consumer.c`, `src/tp_shm.c` | `tests/test_tp_pollers.c` | Partial | Announce cadence policy not enforced |
| W-15.16a-1 | 15.16a | File-backed SHM prefault/lock/fsync policy | n/a | n/a | Partial | File-backed supported, policy not enforced |
...
| W-15.20-1 | 15.20 | Compatibility matrix (layout/wire) | n/a | n/a | Partial | No explicit matrix enforcement |
...
| D-4.4-1 | 4.4 | Lease keepalive send/expiry handling | `src/tp_driver_client.c` | `tests/test_tp_driver_client.c` | Partial | No built-in scheduler |
