...
| W-10.1.2-2 | 10.1.2 | ConsumerHello per-consumer request validity (channel/stream must be both set or both absent) | `src/tp_control.c`, `src/tp_consumer_registry.c` | `tests/test_tp_consumer_errors.c` | Compliant | Outbound validation enforced in `tp_consumer_send_hello` |
...
| W-15.7-2 | 15.7 | Monotonic timestamp when both SlotHeader and FrameDescriptor timestamps present | `src/tp_producer.c` | `tests/test_tp_producer_claim.c` | Compliant | Default timestamps use monotonic clock; caller-supplied capture timestamps must remain monotonic |
