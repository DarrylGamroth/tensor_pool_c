...
**Version History**
- v1.2 (2026-01-12): Remove `headerIndex` from `FrameDescriptor`/`FrameProgress` and derive slot indices from `seq`; rename `FrameProgress.frame_id` to `seq`. Clarify file-backed `mmap` guidance. Update `stride_bytes` validation to require 64-byte multiples (no power-of-two requirement). Schema version fields remain unchanged.
...
- v1.2: same slot count as header ring; `payload_slot = header_index` (header_index derived from `seq`; decoupled/free-list mapping is a v2 change).
  - Choose `stride_bytes` as a multiple of 64 bytes (e.g., 64 B, 256 B, 640 KiB, 1 MiB, 4 MiB, 8 MiB).
...
- `stride_bytes` is explicit and MUST NOT be inferred from page size. It MUST be a multiple of 64 bytes.
