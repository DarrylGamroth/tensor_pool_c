...
- [x] 1) TOCTOU-safe mapping in `tp_shm_map` (MUST)
...
- [x] 2) Canonicalize and cache allowed base dirs (MUST)
...
- [x] 3) Absolute path validation (MUST)
  - Keep strict absolute-path check in `tp_shm_uri_parse()`; ensure it remains a hard error.

- [x] 4) Driver canonical layout generation (MUST for producer-side)
...
- [x] 5) Tooling alignment (SHOULD, REQUIRED if used for provisioning)
...
- [x] 7) Documentation updates (MUST)
